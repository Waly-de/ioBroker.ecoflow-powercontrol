...
class EcoflowMqtt {
    ...
    async testConnection() {
        ...
        await this._testMqttConnect(15000);
        this.adapter.log.info('EcoFlow MQTT test: MQTT broker connection successful.');
    ...
    async _connect() {
        ...
        const brokerUrl = `${protocol}://${d.URL}`;
        const options = {
            port: Number(d.Port),
            clientId: d.clientID,
            username: d.User,
        ...
        this.adapter.log.info(`EcoFlow MQTT: Connecting to broker ${brokerUrl}:${d.Port} (clientId=${d.clientID})...`);
    ...
    _testMqttConnect(timeoutMs = 15000) {
        ...
        this.adapter.log.info(`EcoFlow MQTT test: Connecting to ${brokerUrl}:${d.Port}...`);

        return new Promise((resolve, reject) => {
            let finished = false;
            const options = {
                port: Number(d.Port),
                clientId: `${d.clientID}_test`,
                username: d.User,
                password: d.Passwort,
                protocol,
                reconnectPeriod: 0,
            ...
            const timer = setTimeout(() => {
                done(new Error(`MQTT connect timeout after ${timeoutMs} ms`));
            }, timeoutMs);

            client.once('connect', () => done());
            client.once('error', err => done(new Error(`MQTT connect error: ${err.message}`)));
            client.once('close', () => {
                if (!finished) {
                    done(new Error('MQTT socket closed before connect.'));
                }
            });
        });
    }

...
class EcoflowMqtt {
    ...
    constructor(adapter) {
        ...
        this.cfg = adapter.config.ecoflow;
    ...
    async stop() {
        ...
        this.isConnected = false;
        this.adapter.log.info('EcoFlow MQTT: Stopped.');
    }
    ...
    async _authenticate(email, password) {
        ...
        this.adapter.log.debug(`EcoFlow MQTT: Authenticated as UserID=${userid}`);
    }
    ...
    async _connect() {
        ...
        this.client = mqtt.connect(`mqtt://${d.URL}`, options);

        this.client.on('connect', async () => {
            this.isConnected = true;
            await this.adapter.setStateAsync('info.connection', true, true);
            this.adapter.log.info('EcoFlow MQTT: Connected to broker.');
        ...
        this.client.on('close', async () => {
            this.isConnected = false;
            await this.adapter.setStateAsync('info.connection', false, true);
            this.adapter.log.debug('EcoFlow MQTT: Disconnected.');
        });
    ...
    _subscribeDevices() {
        ...
        for (const device of (this.cfg.devices || [])) {
            ...
            this.client.subscribe(`/app/${uid}/${sn}/thing/property/set`);
            this.client.subscribe(`/app/${uid}/${sn}/thing/property/get`);
            if (device.subscribe) {
                this.client.subscribe(`/app/device/property/${sn}`);
            }
        }
    }
